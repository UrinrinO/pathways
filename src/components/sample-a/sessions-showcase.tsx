...
export function SessionsShowcase() {
  ...
  return (
    <div className="mt-16 grid gap-12 lg:grid-cols-[1fr_0.85fr] lg:items-start">
      {/* Left — hoverable list */}
          ...
          return (
            <li key={s.n}>
              <button
                ...
                onFocus={() => setActive(i)}
                onClick={() => setActive(i)}
                className="group flex w-full items-center gap-5 border-b border-ink/10 py-6 text-left"
              >
                <span
      ...
      {/* Right — image (crossfades) + changing caption */}
      <div className="order-1 lg:order-2">
        <div className="relative aspect-[4/3] overflow-hidden rounded-3xl bg-paper-2 shadow-[0_30px_70px_-30px_rgba(14,18,24,0.4)]">
          {SESSIONS.map((s, i) => (
        ...
        {/* bottom text — changes with the hovered item */}
