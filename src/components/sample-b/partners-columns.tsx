...
/** Minimal columns; each reveals its photo on hover, over a dark base. */
export function PartnersColumns() {
  return (
    <div className="mt-16 grid divide-y divide-white/10 rounded-3xl border border-white/10 bg-panel/40 lg:grid-cols-3 lg:divide-x lg:divide-y-0">
      {CARDS.map((card) => (
        <div
          key={card.title}
          className="group relative flex min-h-[320px] flex-col items-center px-8 py-12 text-center"
        >
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center">
            <div className="text-soft-grey/70 transition-colors duration-500 group-hover:text-lime">
              {LOGOS[card.n]}
            </div>
            <h3 className="mt-7 max-w-[16ch] text-xl font-bold leading-snug tracking-tight text-snow transition-colors duration-500 group-hover:text-snow">
              {card.title}
            </h3>
            <p className="mt-3 max-w-[26ch] leading-relaxed text-soft-grey transition-colors duration-500 group-hover:text-snow/80">
              {card.desc}
            </p>
            <a
              href="#"
